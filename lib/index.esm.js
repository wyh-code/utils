...
function getBase64ImageSize(base64String) {
    ...
    var stringLength = base64WithoutPrefix.length;
    // Base64 编码字符串可能包含尾随的 '=' 字符（填充字符）。每个 '=' 表示少了一个字符。
    var paddingCount = base64WithoutPrefix.endsWith('==') ? 2 : base64WithoutPrefix.endsWith('=') ? 1 : 0;
    // 实际编码的数据长度等于字符串长度减去填充字符的数量
    var encodedLength = stringLength - paddingCount;
    // 每4个Base64字符包含3个字节的信息，计算字节大小
    var sizeInBytes = Math.ceil((encodedLength / 4) * 3);
    return sizeInBytes;
}
function compressImage(file, options) {
    var _a = options || {}, maxWidth = _a.maxWidth, maxHeight = _a.maxHeight, quality = _a.quality, mime = _a.mime;
    quality = quality || 0.8;
 ...
 */
function formatFileSize(bytes, decimals, base) {
    ...
    if (bytes === 0)
        return '0 Bytes';
    var k = base; // 允许自定义基数，例如 1024 或 1000
...
export { asyncSleep, compressImage, compute, copy, copyAsync, dataURItoBlob, debounce, downloadFile, exitFullScreen, formatBankCardNumber, formatFileSize, getBase64ImageSize, getBeginDayOfMonth, getCookies, getDataType, getDaysOfMonth, getImageSize, getLocalStorage, getMonthDays, getMonthInfo, getParentNode, getSessionStorage, getUrlParams, hasNotNullValue, hasNullValue, hexRegex, hexToRgba, hslRegex, hslToRgb, hslaRegex, hslaToRgba, imageToBase64, isDate, isLeapYear, numberWithCommas, packUrlParams, requestFullscreen, rgbaRegex, rgbaToHex, rgbaToHsl, setLocalStorage, setSearchParams, setSessionStorage, sleep, throttle, uniqueByKey };
